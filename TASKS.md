...
- [x] **T0.1** 40k 种子脚本：新增 `scripts/seed-40k.mjs`，可幂等生成约 40000 条维修单（含 items/payments/客户）供对账与压测。验收：脚本运行成功，输出 Repair 总数 ≈ 40000。
...
- [x] **T0.3** search 接口补参数：`/api/repairs/search` 与 `searchRepairs()` 支持 `clientId`、`sourceRepairId`、`technicianKey`（id 或历史姓名或 unassigned）。验收：curl 三种参数各返回正确过滤结果。
...
- [x] **T2.4** ReportsPage 切换到 `/api/reports/overview`，移除本地全量计算。验收：页面数字与切换前一致。
- [x] **T2.5** FinancePage 切换到 `/api/reports/finance`，流水/未收款分页加载。验收：汇总与切换前一致，流水分页可用。
...
- [x] **T6.1** 调用 agent-skills:review 对整个分支做五维审查（正确性/可读性/架构/安全/性能），展示结论。
- [x] **T6.2** 修复审查发现的 critical/high 问题并重跑 T5.5 验证，最终严重问题为 0。
